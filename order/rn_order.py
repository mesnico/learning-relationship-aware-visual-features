...
class RNOrder(OrderBase):
    ...
    def load_features(self,  filename, how_many, indexes):
        f = open(filename, 'rb')
    ...
    def compute_distances(self, query_img_index):
        ...
        if self.distance == 'l2':
            dist_func = utils.l2_dist
        elif self.distance == 'cosine':
            dist_func = utils.dot_dist
        else:
            dist_func = None
        distances = [dist_func(query_feat, f) for f in self.rn_feats]
        if self.preproc=='lsh':
